...
var _MAX_POINT_INTERVAL_MS = 15e3;
var _SECOND_IN_MILLIS = 1e3;
...
L.GPX = L.FeatureGroup.extend({
  ...
  get_duration_string: function(duration, hidems) {
    ...
    if (secs < 10) s += "0";
    s += secs;
    if (!hidems && duration > 0) s += "." + Math.round(Math.floor(duration) * 1e3) / 1e3;
    else s += '"';
    return s;
  },
  ...
  get_moving_speed: function() {
    return this.m_to_km(this.get_distance()) / (this.get_moving_time() / (3600 * 1e3));
  },
  get_moving_speed_imp: function() {
    return this.to_miles(this.m_to_km(this.get_distance())) / (this.get_moving_time() / (3600 * 1e3));
  },
  get_total_speed: function() {
    return this.m_to_km(this.get_distance()) / (this.get_total_time() / (3600 * 1e3));
  },
  get_total_speed_imp: function() {
    return this.to_miles(this.m_to_km(this.get_distance())) / (this.get_total_time() / (3600 * 1e3));
  },
  ...
  get_elevation_data: function() {
    ...
    return this._info.elevation._points.map(
      function(p) {
        return _this._prepare_data_point(
          p,
          _this.m_to_km,
          null,
          function(a, b) {
            return _this._toFixed_helper(a, 2) + " km, " + _this._toFixed_helper(b, 0) + " m";
          }
        );
      }
    );
    ...
    return this._info.elevation._points.map(
      function(p) {
        return _this._prepare_data_point(
          ...
          _this.to_ft,
          function(a, b) {
            return _this._toFixed_helper(a, 2) + " mi, " + _this._toFixed_helper(b, 0) + " ft";
          }
        );
      }
    );
    ...
    return this._info.speed._points.map(
      function(p) {
        return _this._prepare_data_point(
          ...
          _this.ms_to_kmh,
          function(a, b) {
            return _this._toFixed_helper(a, 2) + " km, " + _this._toFixed_helper(b, 2) + " km/h";
          }
        );
      }
    );
    ...
    return this._info.speed._points.map(
      function(p) {
        return _this._prepare_data_point(
          ...
          _this.ms_to_mih,
          function(a, b) {
            return _this._toFixed_helper(a, 2) + " mi, " + _this._toFixed_helper(b, 2) + " mi/h";
          }
        );
      }
    );
    ...
    return this._info.hr._points.map(
      function(p) {
        return _this._prepare_data_point(
          ...
          null,
          function(a, b) {
            return _this._toFixed_helper(a, 2) + " km, " + _this._toFixed_helper(b, 0) + " bpm";
          }
        );
      }
    );
    ...
    return this._info.hr._points.map(
      function(p) {
        return _this._prepare_data_point(
          ...
          null,
          function(a, b) {
            return _this._toFixed_helper(a, 2) + " mi, " + _this._toFixed_helper(b, 0) + " bpm";
          }
        );
      }
    );
    ...
    return this._info.cad._points.map(
      function(p) {
        return _this._prepare_data_point(
          ...
          null,
          function(a, b) {
            return _this._toFixed_helper(a, 2) + " km, " + _this._toFixed_helper(b, 0) + " rpm";
          }
        );
      }
    );
    ...
    return this._info.atemp._points.map(
      function(p) {
        return _this._prepare_data_point(
          ...
          null,
          function(a, b) {
            return _this._toFixed_helper(a, 2) + " km, " + _this._toFixed_helper(b, 0) + " degrees";
          }
        );
      }
    );
    ...
    return this._info.cad._points.map(
      function(p) {
        return _this._prepare_data_point(
          ...
          null,
          function(a, b) {
            return _this._toFixed_helper(a, 2) + " mi, " + _this._toFixed_helper(b, 0) + " rpm";
          }
        );
      }
    );
    ...
    return this._info.atemp._points.map(
      function(p) {
        return _this._prepare_data_point(
          ...
          null,
          function(a, b) {
            return _this._toFixed_helper(a, 2) + " mi, " + _this._toFixed_helper(b, 0) + " degrees";
          }
        );
      }
    );
  ...
  _init_info: function() {
    this._info = {
      name: null,
      length: 0,
      elevation: { gain: 0, loss: 0, max: 0, min: Infinity, _points: [] },
      speed: { max: 0, _points: [] },
  ...
  _load_xml: function(url, cb, options, async) {
    if (async == void 0) async = this.options.async;
  ...
  _parse: function(input, options, async) {
    var _this = this;
    var cb = function(gpx, options2) {
      var layers = _this._parse_gpx_data(gpx, options2);
      if (!layers) {
        _this.fire("error", { err: "No parseable layers of type(s) " + JSON.stringify(options2.gpx_options.parseElements) });
        return;
      }
      _this.addLayer(layers);
      _this.fire("loaded", { layers, element: gpx });
    };
    if (input.substr(0, 1) === "<") {
  ...
  _parse_segment: function(line, options, base_style, polyline_options, tag) {
    ...
    for (var i = 0; i < el.length; i++) {
      ...
      if (_.length > 0) {
        ...
        ll.meta.time = /* @__PURE__ */ new Date("1970-01-01T00:00:00");
      }
      ...
      if (_.length > 0) {
        ...
        ll.meta.speed = time_diff > 0 ? 1e3 * dist_3d / time_diff : 0;
      }
    ...
    if (options.markers.startIcon) {
      ...
      this.fire("addpoint", { point: marker, point_type: "start", element: el[0], line });
      layers.push(marker);
    }
    if (options.markers.endIcon) {
      ...
      this.fire("addpoint", { point: marker, point_type: "end", element: el[el.length - 1], line });
      layers.push(marker);
    }
  ...
  _dist2d: function(a, b) {
    ...
    var r = Math.sin(dLat / 2) * Math.sin(dLat / 2) + Math.cos(this._deg2rad(a.lat)) * Math.cos(this._deg2rad(b.lat)) * Math.sin(dLon / 2) * Math.sin(dLon / 2);
...
//# sourceMappingURL=leaflet-gpx.js.map
